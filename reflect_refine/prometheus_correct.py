...
if __name__ == "__main__":
    ...
    with open("prometheus2-7b-llama3-8b-gold-evaluations.jsonl") as f:
        for row in f:
            row = json.loads(row)
            tiger_evaluations[row["id"]] = row["evaluation"]["feedback"]
            no_errors[row["id"]] = row["evaluation"]["score"] == 5
    ...
    llm = LLM(model="unsloth/llama-3-8b-Instruct", dtype="half", enable_prefix_caching=True)
...
Updated Article: """)
    with open("llama3-8b-test-gold-predictions_prometheus2_correction.jsonl", "w" if not continue_from else "a") as f:
