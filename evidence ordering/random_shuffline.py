...
from dataset import *
import json
from factual_filter_sequence import random_rank_evidences
import random
...
if __name__ == "__main__":
    ...
    for article in tqdm(gold_dataset, total=len(gold_dataset), desc="filtering and ranking"):
        article.evidences = random_rank_evidences(article)
...
Updated Article: """)
    with open(f"llama3-8b-predictions-filter-rank-random-{split}.jsonl", "w" if not resume_from else "a") as f:
