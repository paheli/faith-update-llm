...
ner_pipe = pipeline("ner", model="dslim/bert-base-NER" , aggregation_strategy='average', device_map="cuda:0")


dataset = load_dataset_pkl(f"/fs/scratch/rb_bd_dlp_rng-dl01_cr_AIM_employees/students/tty3kor/fruit/dataset/gold.pkl")
...
with open(file_name) as f:
    for line in tqdm(f, desc="evaluating", total=len(dataset)):
        row = json.loads(line)
        source_target_similarity.append(scorer_rouge.score(row["source"], row["target"])[similarity_score_type].fmeasure)
        source_prediction_similarity.append(scorer_rouge.score(row["source"], row["prediction"])[similarity_score_type].fmeasure)
        article = dataset[row["id"]]
        recall = sum(1 for e in article.target_entities if article.normalised_target[e['start']: e['end']].lower().strip() in row["prediction"].lower()) / len(article.target_entities)
        ...
        evidence_count_scores[len(article.evidences)].append(score[score_type].fmeasure)

        entities = {e["word"].lower().strip() for e in ner_pipe(row["prediction"])} # type: ignore
        target_entities = {e["word"].lower().strip() for e in ner_pipe(row["target"])} # type: ignore
        # target_entities = {article.normalised_target[e['start']: e['end']].lower().strip() for e in article.target_entities}

        precision = len(entities & target_entities) / len(entities) if len(entities) else 1
        ...
        supporting_entities = {e["word"].lower().strip() for e in ner_pipe(row["source"])} # type: ignore
        texts = []
        for evidence in article.evidences.values():
            text = evidence.content.raw_text if isinstance(evidence.content, Table) else evidence.content
            texts.append(f"{evidence.title} {evidence.section} {text}")
        
        for b in ner_pipe(texts, batch_size=48): # type: ignore
            supporting_entities |= {e["word"].lower().strip() for e in b} # type: ignore
        ...
        table_percentage = len([e for e in article.evidences.values() if isinstance(e.content, Table)]) / len(article.evidences)
        table_percentages_scores.setdefault(table_percentage, [])
        table_percentages_scores[table_percentage].append(score[score_type].fmeasure)

        required_edit_length = (len(article.normalised_target) - len(article.normalised_source)) / len(article.normalised_source)
        required_edit_lengths.setdefault(required_edit_length, [])
        required_edit_lengths[required_edit_length].append(score[score_type].fmeasure)

        updated_sentences_count = sum(1 for s in article.updated_article_sentences if isinstance(s, UpdatedSentence)) 
        if updated_sentences_count:
            references_per_sentence = sum(len(s.evidence_indices) for s in article.updated_article_sentences if isinstance(s, UpdatedSentence)) / updated_sentences_count
            evidences_per_update.setdefault(references_per_sentence, [])
            evidences_per_update[references_per_sentence].append(score[score_type].fmeasure)
        
        
aggregate_scores = update_rouge_aggregate(sources, targets, predictions)

table_only_scores = table_percentages_scores[1]
mixed_scores = reduce(lambda a, b: a + b, (v for k, v in table_percentages_scores.items() if k not in [0, 1]), [])
...
for individual_score_type, individual_score in [("update_rouge1", update_rouge1), 
                                                ...
                                                ("update_rougeLsum", update_rougeLsum), 
                                                ("entity_recall", entity_recalls), 
                                                ("entity_precision", entity_precisions),
                                                ("unsupported_entities", unsupported_entities_scores),
                                                ...
                                                (f"table_only_{score_type}", table_only_scores),
                                                (f"mixed_{score_type}", mixed_scores),
                                                (f"text_only_{score_type}", plain_text_only_scores),
                                                (f"appends_{score_type}", appends),
                                                (f"unnecessary_appends_{score_type}", unnecessary_appends)]:
    plt.hist(individual_score, edgecolor='black', bins='auto')  # Adjust bins for desired granularity
...
for label, individual_score_type, individual_score in [("Evidence Count", f"Mean {score_type}", evidence_count_scores),
                                                       ("Tabular Evidence Percentage", f"Mean {score_type}", table_percentages_scores),
                                                       ("Required Edit Length", f"Mean {score_type}", required_edit_lengths),
